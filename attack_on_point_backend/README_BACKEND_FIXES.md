...
The seed stores dataset name, version, source, IP address, data classification, deterministic dependencies, canonical framework mappings, deterministic baseline risk, EAL, VaR, and asset-level driver snapshots.

## Risk calculation

When an AI result omits an asset or returns an unusable zero-valued record, the backend calculates a deterministic fallback for that asset. The fallback is labeled `backend-deterministic-v2` and includes ranked vulnerability, exposure, business-criticality, and observed attack-pressure drivers plus an input snapshot hash.
