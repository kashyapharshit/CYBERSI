...
const saveAiResultsService = async (resultsData = {}) => {
  ...
  const asset_risks = sourceAssets.map((asset) => {
    ...
    if (aiHasUsableRisk) {
      const fallback = calculateRiskForAsset(assetWithObservedPressure(asset), vulnerabilitiesByAsset[asset.asset_id] || []);
      return {
        ...fallback,
        ...aiRisk,
        asset_id: asset.asset_id,
    ...
    return calculateRiskForAsset(assetWithObservedPressure(asset), vulnerabilitiesByAsset[asset.asset_id] || []);
  });
