...
const normalizeTelemetry = (type, rawData = {}, ingestionJobId = '', parsedData = {}) => {
...
const ingestTelemetryService = async (type, rawData, context = {}) => {
  if (Array.isArray(rawData)) {
    return SecurityEvent.insertMany(rawData.map((item) => normalizeTelemetry(type, item, context.job_id)));
  }
  return SecurityEvent.create(normalizeTelemetry(type, rawData, context.job_id));
};
