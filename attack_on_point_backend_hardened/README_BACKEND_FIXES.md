...
The seed stores dataset name, version, source, deterministic baseline risk, EAL, VaR, and asset-level driver snapshots.

## Risk calculation

When an AI result omits an asset or returns an unusable zero-valued record, the backend calculates a deterministic fallback for that asset. The fallback is labeled `backend-deterministic-v1` and includes ranked drivers and an input snapshot hash.
