...
const snapshotHash = (asset, vulnerabilities) => crypto
  .createHash('sha256')
  .update(JSON.stringify(canonicalize({ asset, vulnerabilities })))
  .digest('hex');

const sourceTimestamp = (asset, vulnerability) => vulnerability?.updatedAt || vulnerability?.createdAt || asset?.updatedAt || asset?.createdAt || null;

const calculateRiskForAsset = (asset, vulnerabilities = []) => {
  ...
  const impactInr = Math.max(
    1,
    (Number(asset.total_records || asset.stored_records_count) || 0) * (Number(asset.cost_per_record_inr || asset.cost_per_breached_record_inr) || 0)
      + (Number(asset.hourly_downtime_cost_inr) || 0) * 24
      + (Number(asset.regulatory_penalty_inr) || 0)
      + (Number(asset.reputation_loss_inr) || 0)
  );
  ...
  return {
    ...
    drivers,
    formula: 'EAL=impact x probability x impact_multiplier; p=clamp(0.05+weighted_likelihood x 0.75)',
    model_version: 'backend-deterministic-v2',
    input_snapshot_hash: snapshotHash(asset, openVulnerabilities),
    assessed_at: new Date()
  };
};

module.exports = { calculateRiskForAsset };
