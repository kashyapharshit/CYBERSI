const parseOpenVas = (rawData = {}) => {
  return {
    type: 'openvas',
    asset_id: rawData.asset_id ? String(rawData.asset_id).trim() : '',
    source: rawData.source ? String(rawData.source).trim() : 'openvas-scanner',
    event_type: rawData.event_type
      ? String(rawData.event_type).trim()
      : (rawData.nvt_name || rawData.vulnerability ? String(rawData.nvt_name || rawData.vulnerability).trim() : 'vulnerability_scan'),
    ...
    open_ports: Array.isArray(rawData.open_ports)
      ...
      : (rawData.port ? [Number(rawData.port)].filter((p) => !isNaN(p)) : []),
    services: Array.isArray(rawData.services)
      ? rawData.services.map((s) => String(s).trim())
      : (rawData.service ? [String(rawData.service).trim()] : []),
