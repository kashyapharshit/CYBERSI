...
import { api, compactMoney, formatDate, isDemoMode } from './api';
...
export function AuditPageV2({ data, canTamper = true }) {
  ...
  const [busy, setBusy] = useState(false);
  const verify = async () => { setBusy(true); try { setVerification(await api.verifyAudit()); } catch (err) { setVerification({ status: 'ERROR', message: err.message }); } finally { setBusy(false); } };
