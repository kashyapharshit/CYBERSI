...
Monte Carlo combines CVSS, EPSS, CISA KEV, patch age, internet exposure, and business criticality. It simulates annual loss per asset and returns both asset VaR and an aggregate portfolio 95th-percentile VaR. `MONTE_CARLO_SEED` makes the result repeatable for the same input. The canonical contract accepts both top-level `internet_exposed` and the legacy nested feature field.

The optimizer uses the supplied budget and control costs. It maximizes expected INR risk reduction and applies the same overlap factor and 85% maximum combined reduction used by the Node what-if endpoint. The optimizer does not silently return a fake result when there are no controls or no budget.
