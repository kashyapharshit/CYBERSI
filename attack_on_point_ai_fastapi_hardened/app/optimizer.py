...
def _items(payload: dict[str, Any], asset_eal_override: dict[str, float] | None = None) -> list[dict[str, Any]]:
    ...
    for control in payload.get("candidate_controls", payload.get("controls", [])):
        ...
        result.append({"control": control, "cost": cost, "reduction": reduction, "target_eal": target_eal, "value": value})
    return result
...
def severity_only_baseline(payload: dict[str, Any], options: dict[str, Any] | None = None) -> dict[str, Any]:
    """Severity-only selection with actual EAL outcome reporting."""
    options = options or payload.get("optimizer_options") or {}
    budget = int(float(options.get("budget_inr", payload.get("enterprise_budget_inr", 0)) or 0))
    ...
    for asset in payload.get("assets", []):
        open_vulnerabilities = [
            item for item in asset.get("vulnerabilities", [])
            if item.get("status") != "fixed"
        ]
        asset_severity[str(asset.get("asset_id"))] = max(
            (float(item.get("cvss", 0) or 0) / 10 for item in open_vulnerabilities),
            default=0.0
        )
    ...
    for control in payload.get("candidate_controls", payload.get("controls", [])):
        ...
        reduction = _reduction(
            control.get("risk_reduction_pct", control.get("effectiveness", 0))
        )
        target = str(control.get("target_asset_id") or "GLOBAL")
    ...
    candidates.sort(
        key=lambda item: (
            item["severity_value"] / item["cost"]
            if item["cost"] else item["severity_value"]
        ),
        reverse=True
    )
    ...
    # Selection is severity-only, but outcome is measured using actual Monte Carlo EAL.
    analysis = analyze_payload(payload)
    total_eal = sum(
        float(item.get("eal_inr", 0) or 0)
        for item in analysis.get("asset_risks", [])
    )

    combined_reduction = _bounded_reduction(selected)
    estimated_risk_reduction = total_eal * combined_reduction
    estimated_residual_eal = max(0, total_eal - estimated_risk_reduction)
    ...
    return {
        ...
        "recommended_control_ids": [
            control.get("control_id") for control in selected_controls
        ],
        "recommended_controls": selected_controls,
        ...
        "estimated_residual_eal_inr": round(estimated_residual_eal),
        "assumptions": {
            ...
            "outcome_measurement": "Monte Carlo EAL with severity-only control selection"
        }
    }