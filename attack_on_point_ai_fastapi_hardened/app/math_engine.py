...
def _signals(asset: dict[str, Any], vulnerabilities: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    internet_exposure = 1.0 if (
        asset.get("internet_exposed")
        or asset.get("is_internet_facing")
        or asset.get("features", {}).get("internet_exposed")
    ) else 0.0

    criticality = _criticality_weight(asset)

    failed_auth_count = number(
        asset.get("features", {}).get(
            "failed_auth_count",
            asset.get("failed_auth_count", 0)
        )
    )

    # 1,000 failed attempts represents maximum observed attack pressure.
    attack_pressure = clamp(failed_auth_count / 1000.0)

    signals = [
        {
            "factor": "CVSS exploitability",
            "value": max_cvss,
            "weight": 0.30,
            "source": "vulnerability.cvss",
            "source_timestamp": _source_timestamp(asset, max_cvss_item),
            "explanation": "Highest open CVSS score across the asset findings."
        },
        {
            "factor": "EPSS likelihood",
            "value": max_epss,
            "weight": 0.25,
            "source": "vulnerability.epss",
            "source_timestamp": _source_timestamp(asset, max_epss_item),
            "explanation": "Highest EPSS score across the asset findings."
        },
        {
            "factor": "CISA KEV exposure",
            "value": kev_ratio,
            "weight": 0.20,
            "source": "vulnerability.cisa_kev",
            "source_timestamp": _source_timestamp(asset),
            "explanation": "Share of open findings listed in CISA KEV."
        },
        {
            "factor": "Patch age",
            "value": patch_age,
            "weight": 0.10,
            "source": "vulnerability.patch_age_days",
            "source_timestamp": _source_timestamp(asset, max_patch_item),
            "explanation": "Oldest open finding age normalized to 180 days."
        },
        {
            "factor": "Internet exposure",
            "value": internet_exposure,
            "weight": 0.10,
            "source": "asset.internet_exposed",
            "source_timestamp": _source_timestamp(asset),
            "explanation": "Whether the asset is reachable from the internet."
        },
        {
            ...
            "source_timestamp": _source_timestamp(asset),
            "explanation": "Business criticality multiplier for impact."
        }
    ]

    if attack_pressure > 0:
        signals.append({
            "factor": "Observed attack pressure",
            "value": round(attack_pressure, 4),
            "weight": 0.10,
            "source": "telemetry.failed_auth_count",
            "source_timestamp": _source_timestamp(asset),
            "explanation": "Capped failed-authentication pressure from telemetry; this is not proof of compromise."
        })
...
def calculate_asset_risk(
    ...
    return {
        ...
        "drivers": drivers,
        "formula": "EAL=mean(simulated_loss); VaR=P95(simulated_loss); p=clamp(0.05+weighted_likelihood*0.75)",
        "model_version": "monte-carlo-v2",
